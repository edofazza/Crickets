...
class GAloop:
    def __init__(self, pop_size=5, p_co=0.9, p_mut=0.5, max_gen=2, hof=1, crowding_fa=20.0):
    ...
    def run(self, train_set_path: str, train_l_path: str, val_set_path: str, val_l_path: str):
        train_set = np.load(train_set_path)
        train_labels = np.load(train_l_path)
        val_set = np.load(val_set_path)
        val_labels = np.load(val_l_path)
